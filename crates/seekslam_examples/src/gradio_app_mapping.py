...
def run_model(target_dir, model) -> dict:
    """
    在 'target_dir/images' 文件夹中的图像上运行 VGGT 模型并返回预测结果。
    """
    ...
    for key in predictions.keys():
        if isinstance(predictions[key], torch.Tensor):
            predictions[key] = predictions[key].cpu().numpy().squeeze(0)  # 移除批量维度
...
# -------------------------------------------------------------------------
# 2) 处理上传的视频/图像 --> 生成 target_dir + images
# -------------------------------------------------------------------------
def handle_uploads(input_video, input_images):
    """
    创建一个新的 'target_dir' + 'images' 子文件夹，并将用户上传的图像或从视频中提取的帧放入其中。返回 (target_dir, image_paths)。
    """
    ...
    if input_video is not None:
        ...
        frame_interval = int(fps * 1)  # 每秒 1 帧
...
def update_gallery_on_upload(input_video, input_images):
    """
    每当用户上传或更改文件时，立即处理它们并在图库中显示。返回 (target_dir, image_paths)。
...
def gradio_demo(
    ...
    """
    使用已经创建的 target_dir/images 进行重建。
    """
...
# -------------------------------------------------------------------------
# 5) 用于 UI 重置和重新可视化的辅助函数
# -------------------------------------------------------------------------
def clear_fields():
    """
    清除 3D 查看器、存储的 target_dir 并清空图库。
    """
    return None
...
def update_visualization(
    ...
    """
    从 npz 文件中重新加载保存的预测结果，为新参数创建（或重用）GLB 文件，并将其返回给 3D 查看器。如果 is_example == "True"，则跳过。
    """
...
with gr.Blocks(
    ...
    def example_pipeline(
        ...
        """
        1) 将示例图像复制到新的 target_dir
        2) 进行重建
        3) 返回 model3D + 日志 + 新目录 + 更新后的下拉菜单 + 图库
        我们不返回 is_example。它只是一个输入。
        """
        target_dir, image_paths = handle_uploads(input_video, input_images)
        # 示例中始终使用 "All" 进行帧过滤
        frame_filter = "All"
    ...
    #  - 清除字段
    #  - 更新日志
    #  - 使用现有的 target_dir 调用 gradio_demo(...)
    #  - 然后将 is_example 设置为 "False"
    # -------------------------------------------------------------------------
    submit_btn.click(fn=clear_fields, inputs=[], outputs=[reconstruction_output]).then(
        ...
        fn=lambda: "False", inputs=[], outputs=[is_example]  # 将 is_example 设置为 "False"
    )
    ...
    demo.queue(max_size=20).launch(show_error=True, share=True, ssr_mode=False)(base) 
