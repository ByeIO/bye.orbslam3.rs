...
class _FlightMission:
    ...
    def __init__(self):
        ...
        # 原子任务
        self.tasks = _MissionTasks(self.drone, self.controller)
    ...
    def run(self):
        ...
        while self.drone.step(self.drone.time_step) != -1:
            ...
            ###############################################   
                ...
                print_once("准备自旋")
                # if (imgs_base64_list, map_base64) = self.tasks.mapping() == True:
                self.mission_state = _MissionState.HOVER
            ###############################################       
            elif self.mission_state == _MissionState.HOVER:
                print_once("等待决策模块指令")
                # 3. 保持悬停在1.5m, 等待决策模块的指令然后才切换状态
