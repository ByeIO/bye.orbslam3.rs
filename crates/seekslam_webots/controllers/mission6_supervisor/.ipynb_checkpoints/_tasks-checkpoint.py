...
## 字母目标识别
from _ocr import ocr_img_with_box_and_save
## 深度估计
from _depth import depth_img_and_save
#############################################
...
class _MissionTasks():
    def __init__(self, drone_super, controller, agent):
    ...
    def mapping_task(self) -> bool:
        ...
        try:
            ...
            return False
    #############################################    
    def ocr_task(self) -> (bool, list):
        '''字母识别任务'''
        # 调用_ocr的对应函数
        # 采集图像
        img_data_base64 = self.camera.capture_image_and_save_and_return()
        # ocr获取的数组或者错误
        ocr_result_list = []
        if img_data_base64:
            try:
                ocr_result_list = ocr_img_with_box_and_save(img_data_base64)
                return (True, ocr_result_list)
            except Exception as e: 
                print(f"可能没有发现目标，错误信息: {str(e)}")
                return (False, None)
        # 图片获取失败导致的ocr失败
        return (False, None)
    ##############################################
    ...
    def _debug_full(self) -> bool:
        ...
        if(self.drone.step(self.drone.time_step) != -1):
            ...
            # 搜索目标
            ocr_result = ''
            if self.drone.step(self.drone.time_step) != -1:  # 补全括号
                (is_ok, result) = self.ocr_task()
                if is_ok and any(self.target in s for s in result):  # 检查目标是否在任一字符串中
                    print("找到目标")
                else:
                    print("继续搜索目标")
            # 深度估计(避障)
            depth_result = ''
            if self.drone.step(self.drone.time_step) != -1:
                (is_ok, result) = self.depth_task()
                if is_ok and result:
                    print("深度估计成功, 将会决策搜索路线以及避障")
                else:
                    print("深度估计失败, 但是仍然需要继续任务")
            # 决策
            self.ollama_task(stage=2)
            # 不要太快了
            time.sleep(1)
        if(self.drone.step(self.drone.time_step) != -1):
            # 右转90度
        ...
        if(self.drone.step(self.drone.time_step) != -1):
            ...
            if self.drone.step(self.drone.time_step) != -1:
                (is_ok, result) = self.depth_task()
                if is_ok and result:
                    print("深度估计成功, 判断与目标距离")
                else:
                    print("深度估计失败, 但是仍然需要继续任务")
            # 决策
            self.ollama_task(stage=4)
            # 模拟攻击
    ...
    def depth_task(self) -> (bool, str):
        ...
        if img_data_base64:
            try:
                depth_result_str = depth_img_and_save(img_data_base64)
                return (True, depth_result_str)
            except Exception as e: 
                print(f"可能没有发现目标，错误信息: {str(e)}")
                return (False, None)
        # 图片获取失败导致的depth失败
    ...
    ##############################################                  
    def ollama_task(self, stage) -> bool:
        '''核心任务, 智能决策并形成控制指令'''
        # 1. 获取相机图像base64
        img_base64 = ''
        if self.drone.step(self.drone.time_step) != -1:
            img_base64 = self.camera.capture_image_and_save()
        ...
        # 4. 调用_ollama模块的对应函数进行决策
        if self.drone.step(self.drone.time_step) != -1: 
            self.agent.process_stage(stage, image_base64=img_base64, depth_imgs_base64=depth_result, ocr_result=ocr_result)
    ##############################################
    def _debug_mapping(self) -> str:
        '''使用四张图片建图'''
        pass
    ##############################################